        ...
        if (messageObj.containsKey('status')) {
          ...
          if (status == 200) {
            ...
            if (status == 401) {
              ...
              return ApiResponse.failure(message ?? 'Authentication failed', status: 401);
            }
...
void _handleAuthenticationError() {
  try {
    if ($sl.isRegistered<AuthCubit>()) {
      ...
      $logger.info('[ResponseParser] User logged out due to authentication error');
    }
  } catch (e, st) {
    $logger.error('[ResponseParser] Error during logout', e, st);
  }
}
